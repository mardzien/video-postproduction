...
try:
    # When used as module: python -m mido.midi_import ...
    from .generator import MelodyGenerator  # type: ignore[no-redef]
except ImportError:
    # When used as script from within mido/: python midi_import.py ...
    from generator import MelodyGenerator  # type: ignore[no-redef]
...
def _iter_track_messages_with_abs_time_sec(
    mid: mido.MidiFile, track_index: int
) -> Iterable[tuple[float, mido.Message]]:
    """
...
def _collect_track_stats(mid: mido.MidiFile) -> list[TrackStats]:
...
def _track_has_tempo(track: mido.MidiTrack) -> bool:
    return any(msg.type == "set_tempo" for msg in track)


def _build_midi_subset(mid: mido.MidiFile, track_indices: list[int]) -> mido.MidiFile:
...
def cmd_analyze(args: argparse.Namespace) -> int:
    mid = mido.MidiFile(str(Path(args.midi)))
    stats = _collect_track_stats(mid)
